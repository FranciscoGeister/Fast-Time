...
<div class=container>
  <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active" aria-current="page"><a>Agendar Hora</a></li>
        </ol>
  </nav>
  <br>
  {{Form::open(['url' => 'store_cite', 'method' => 'post', 'role' =>  'form'])}}
  <select class="form-control" name="sede" id="sede">
    <option value=9999 selected>Seleccionar Maquina</option>
    <option value=8888 >Mostrar Todo</option>
    @foreach($machines as $machine)
    <option value="{{$machine->id}}">{{ $machine->sede_nombre }} - {{$machine->nombre}}</option>
    @endforeach
  </select>
  @can('create',App\Hora::class)
            ...
            <div class="row">
              ...
              <div class="col-md-6">
                 <div class="col-md-12">
                   ...
                   <div class="form-group"> 
                     ...
                     {{ Form::text('debt', '', ['class' => 'form-control', 'id' => 'debt'])}}
                   </div>
                 </div>
              </div>
              <!--END LEFT-->
              ...
              <div class="col-md-6">
                 <div class="col-md-12">
                   ...
                   <option value=9999 selected disabled>Ninguna</option>
                   @foreach($status as $s)
...
<script>
  ...
  $("#_socio").change(function(){
    ...
    var deuda = document.getElementById("debt");
    removeOptions(y);
    function removeOptions(selectbox)
    {
        var i;
    ...
    if ($('#_socio').val().length > 3) {
      ...
      if(data.sesiones.length == 0){
            var option = document.createElement("option");
            option.text = 'Cortesía Especial';
            //codigo para sesion de cortesia fuera de plan
            option.value = 'Cortesía Especial';
            y.add(option);
      }
      else{
        for (var i = 0; i < data.sesiones.length; i++) {
          if(i%2 == 0){
            var option = document.createElement("option");
            option.text = data.sesiones[i]+' ('+data.sesiones[i+1]+')';
            option.value = data.sesiones[i];
            y.add(option);
          }
        }
      }
      deuda.value = formatNumber.format(data.deuda,"$");
      });
    }
  });
  ...
  $("#sede").change(function(){
    ...
    var events= {
                  url: BASEURL + '/event_machine/'+$('#sede').val(),
                  type: 'GET',
    ...
    $('#calendar').fullCalendar('addEventSource', hours);
  });
  ...
  var options = {
  	...
  	getValue: function(element) {
              member = document.getElementById("member_id");
              $(member).val(element.id);
              console.log(member);
